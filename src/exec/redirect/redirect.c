...
pid_t	process_cmd(t_shell *shell, int *wait_pid)
{
	...
	if (pid == 0)
		redirect_child(shell, pipefd, wait_pid);
	if (shell->prev_fd != -1)
		close(shell->prev_fd);
	close(pipefd[1]);
...
int	process_redirect(t_shell *shell, int *wait_pid, int j)
{
	int	i;
	...
	if ((shell->cmd[i] == NULL || !ft_strcmp(shell->cmd[i], "|")))
		return (process_cmd(shell, wait_pid));
	return (1);
}
